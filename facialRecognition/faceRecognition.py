...
subjects = ["", "Ramiz Raja", "Elvis Presley", "David Peng", "Tiger Peng"]
...
def detect_face(img):
    ...
    face_cascade = cv2.CascadeClassifier('opencvFilter/lbpcascade_frontalface.xml')
...
def predict(test_img, name):
    ...
    if face is None:
        ...
        label_text = "can not detect face"
        #confidence = 200
        #label = 0
        return img, isFace
    else:
    ...
    if confidence < 50 and subjects[label] == name:
        label_text = subjects[label]
        isFace = True
    else:
        label_text = "Not " + name
    print("label %s confidence %s" %(label, confidence))

    #draw a rectangle around face detected
    draw_rectangle(img, rect)
    print("label %s label_text %s" %(label, label_text))
...
test_img3 = cv2.imread("test-data/test5.jpg")
#test_img3 = cv2.imread("instantCheck.jpg")
test_img4 = cv2.imread("test-data/test7.jpg")

#perform a prediction
#predicted_img1, isFace1 = predict(test_img1, subjects[1])
#predicted_img2, isFace2 = predict(test_img2, subjects[2])
#predicted_img3, isFace3 = predict(test_img3, subjects[3])
#predicted_img4, isFace4 = predict(test_img4, subjects[4])
...
# real time capture picture from webcam and recognize it with name
print("start to detect & recognize face from webcam")
log.basicConfig(filename='InstantCheck.log',level=log.INFO)
...
while True:
    if matchFace:
        print("find the matching face, congratulations!!!!")
        break;
    ...
    while ret:
        videoCapture.set(cv2.CAP_PROP_POS_MSEC,(count*50000))
        ret, frame = videoCapture.read()
        #cv2.imwrite("instantCheck%d.jpg" % count, frame)
        cv2.imwrite("instantCheck.jpg", frame)
        log.info("detect and recognize new face image from webcam")
        print("detect and recognize new face image from webcam")

        # save frame as picture
        realtimeFrame = cv2.imread("instantCheck.jpg")

        #perform a prediction
        predictedImage, matchFace = predict(realtimeFrame, subjects[3])

        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

        faceCascade = cv2.CascadeClassifier('opencvFilter/lbpcascade_frontalface.xml')
        ...
        if anterior != len(faces):
            ...
            log.info("no face detected from webcam")
        ...
        if matchFace:
            cv2.imshow(subjects[3], cv2.resize(predictedImage, (400, 500)))
            #break
        else:
